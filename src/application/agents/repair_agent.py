...
from src.application.agents.reflection_agent import ReflectionReport, ReflectionAgent
from src.infrastructure.sandboxes.base_sandbox import BaseSandbox, SandboxResult
    ...
    def apply_repair(self, report: ReflectionReport, source_code: Dict[str, str], sandbox: BaseSandbox, max_retries: int = 3) -> Dict[str, str]:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            if verify_res.exit_code == 0:
                # Perform formal semantic verification and safety checks via UnifiedVerificationEngine (RFC-015)
                from src.domain.verification.verification_engine import UnifiedVerificationEngine
                verification_engine = UnifiedVerificationEngine.get_instance()
                ver_report = verification_engine.verify_patch(
                ...
                self.logger.info(
                    f"Surgical repair succeeded on attempt {attempt}! "
                    f"Formal verification passed: {ver_report.verification_passed} (Risk level: {ver_report.risk_score.risk_level}, score: {ver_report.risk_score.numerical_score})"
                )
                return source_code
